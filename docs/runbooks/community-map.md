...
Labels prioritize the selected channel, hover, then the largest visible audiences. Selection includes connected channels' names. The label budget grows with zoom, text widths are measured using the rendered font, and collision checks run in screen-sized units. Offscreen channels do not consume the budget, including the SVG's wide or tall margins. Text and outlines stay the same screen size while zooming. The search panel can hide unconnected channels; selecting one from search still reveals it.
